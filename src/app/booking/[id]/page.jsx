...
const BookingPage = () => {
  ...
  const regions = useMemo(() => [...new Set(locations.map((loc) => loc.region))], [locations]);
  const filteredDistricts = useMemo(() => locations.filter((loc) => loc.region === selectedRegion), [locations, selectedRegion]);
  const filteredAreas = useMemo(() => {
    const districtData = filteredDistricts.find((loc) => loc.district === selectedDistrict);
  ...
  const handleBooking = async (e) => {
    ...
    const bookingData = {
      ...
      location: { region: selectedRegion, district: selectedDistrict, area: selectedArea, address: address },
      totalCost,
      status: "Pending",
      timestamp: new Date(),
    };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10 text-center lg:text-left space-y-2">
          <h1 className="text-3xl md:text-5xl font-black text-base-content tracking-tighter">
            Complete Your <span className="text-primary text-opacity-80">Booking</span>
          </h1>
          <p className="text-base-content/60 font-medium">Please provide your details to confirm the service.</p>
        </div>
          ...
          <div className="lg:w-2/5 bg-gradient-to-br from-primary to-primary/80 p-8 md:p-12 text-white relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
                ...
                <div className="space-y-8">
                  <div className="flex items-start gap-5">
                    <div className="w-14 h-14 rounded-2xl bg-white/20 flex items-center justify-center text-3xl shadow-inner border border-white/10">
                      {service.title.includes("Baby") ? "👶" : service.title.includes("Elderly") ? "👴" : "🏥"}
                    </div>
                    <div>
                      <p className="text-xs text-white/60 font-black uppercase tracking-widest">Service</p>
                      <h3 className="text-2xl font-bold leading-tight">{service.title}</h3>
                    </div>
                  </div>

                  <div className="flex items-start gap-5">
                    <div className="w-14 h-14 rounded-2xl bg-white/20 flex items-center justify-center text-2xl shadow-inner border border-white/10">💰</div>
                    <div>
                      <p className="text-xs text-white/60 font-black uppercase tracking-widest">Rate</p>
                      <h3 className="text-2xl font-bold">${service.price}<span className="text-sm font-medium opacity-50">/hr</span></h3>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-16 bg-black/20 rounded-[2rem] p-8 border border-white/10 backdrop-blur-md">
                 <p className="text-xs font-black uppercase tracking-widest opacity-60 mb-1">Final Amount</p>
                 <div className="flex justify-between items-end">
                    <p className="text-5xl font-black tracking-tighter">${totalCost}</p>
                    <p className="text-[10px] font-bold opacity-40 uppercase">Tax Incl.</p>
                 </div>
              </div>
            </div>
          </div>
            ...
            <form onSubmit={handleBooking} className="space-y-8">
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3">
                  <span className="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center font-black text-sm">1</span>
                  <h3 className="font-black text-base-content text-lg tracking-tight">Location Details</h3>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="form-control">
                    <label className="label-text font-bold text-base-content/70 mb-2 ml-1">Division</label>
                    <select
                  ...
                  <div className="form-control">
                    <label className="label-text font-bold text-base-content/70 mb-2 ml-1">District</label>
                    <select
                      className="select select-bordered w-full rounded-2xl h-14 border-base-300 focus:ring-4 focus:ring-primary/10 bg-base-200 text-base-content font-bold"
                ...
                <div className="form-control">
                  <label className="label-text font-bold text-base-content/70 mb-2 ml-1">Area / Upazila</label>
                  <select
                    className="select select-bordered w-full rounded-2xl h-14 border-base-300 focus:ring-4 focus:ring-primary/10 bg-base-200 text-base-content font-bold"
                    value={selectedArea}
                ...
                <div className="form-control">
                  <label className="label-text font-bold text-base-content/70 mb-2 ml-1">Detailed Street Address</label>
                  <textarea
                ...
                <div className="flex flex-col md:flex-row items-end gap-5">
                  ...
                  <div className="w-full md:w-1/3">
                    <label className="label-text font-black text-primary mb-2 flex items-center gap-2 uppercase tracking-widest text-[10px]">
                      <span className="w-5 h-5 rounded-md bg-primary text-white flex items-center justify-center">2</span>
                      Set Duration
                    </label>
                    <div className="relative">
                      <input
                        ...
                        required
                      />
                      <span className="absolute right-4 top-5 text-[10px] font-black text-base-content/30 uppercase tracking-widest">Hrs</span>
                    </div>
                  </div>
